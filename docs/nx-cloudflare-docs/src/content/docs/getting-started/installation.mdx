...
3. **Verify the plugin is registered**
   ...
   The output should list `serve`, `deploy`, `typegen`, `version-upload`,
   `version-deploy`, `tail`, `secret`, and `version-secret` targets (plus a
   `d1` target if the project's Wrangler config declares D1 databases). If you
   don't have a Worker yet, the [Quick start](/getting-started/quick-start)
