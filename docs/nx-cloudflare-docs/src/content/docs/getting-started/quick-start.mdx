...
In this tutorial, you'll create a Cloudflare Worker, serve it locally, and deploy
it to Cloudflare — all from an Nx workspace.

## Before you start

- **Node.js 22 or later** — an active LTS release. Required by Nx and Wrangler.
- **A Cloudflare account** — needed for deployment. Sign up at
  [dash.cloudflare.com](https://dash.cloudflare.com/sign-up).
- **Wrangler v4** — installed automatically when you add the plugin.
...
1. **Add the plugin to your workspace**
   ...
   Use `nx add`, which installs the package and runs the
   ...
   This installs Wrangler v4, `@cloudflare/workers-types`, and Vitest — see
   [Installation](/getting-started/installation) for the full list of what gets
   added.

2. **Generate a Worker**

   Scaffold a hello-world Worker using the
...
3. **Serve the Worker locally**
   ...
   Now run the Worker in a local dev server:
...
This lists `serve`, `deploy`, `typegen`, `version-upload`, `version-deploy`,
`tail`, `secret`, and `version-secret`, confirming the plugin registered the
project.
...
- [Inferred targets reference](/inferred-targets) — `serve`, `deploy`, `typegen`,
  `version-upload`, `version-deploy`, `tail`
