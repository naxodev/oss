...
Use the [`worker-config` generator](/guides/generators-worker-config):
...
<Aside>
  These generators only edit `wrangler.jsonc`/`wrangler.json`. A Worker on
  `wrangler.toml` must be converted first.
</Aside>
...
- [worker-config generator](/guides/generators-worker-config) — full option reference
